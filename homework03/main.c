 ...
 */
  ...
  // Create a histogram of size m bc we need to count rows
	unsigned int hist[m + 1];
	
	// Initialize all indices of histogram to 0
	for (int i = 0; i < m + 1; i++) {
		hist[i] = 0;
	}
	
	// Populate the buckets with the number of times a row appears
	for (int i = 0; i < nnz; i++) {
		hist[row_ind[i] - 1] += 1;
	}
	
	// Calculate the vals for each index of csr_row_ptr and store them
	for (int i = 0; i < m + 1; i++) {
		// Start at index 1
		if (i == 0) {
			(*csr_row_ptr)[i] = 1;
		...
		unsigned int val = (*csr_row_ptr)[i-1] + hist[i-1];
		(*csr_row_ptr)[i] = val;
	}

	// make  copy of csr_row_ptr	
	...
	// Build the csr_col_ind and the csr_vals arrays
	for (int i = 0; i < nnz; i++) {
		// obtain  index value to use
		unsigned int index = temp[row_ind[i] - 1];

		// increment  index pointer
		temp[row_ind[i] - 1]++;

		// store the column value at the appropriate index in csr_col_ind
		(*csr_col_ind)[index - 1] = col_ind[i];

		// store the value at the appropriate index in csr_vals
		(*csr_vals)[index - 1] = val[i];
	}
}
 ...
 */
  ...
  {
	for (int i = 0; i < m; i++) {
		// Obtain the begin and end indices
